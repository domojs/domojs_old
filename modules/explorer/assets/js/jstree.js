(function ($) {
    ...
    var refresh = function (treeNode) {
        ...
        $.ajax({
            type: 'get',
            url: treeNode.data("node").url,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            processdata: true,
            cache: false,
            success: function (msg) {
                UpdateNode(treeNode, msg || msg.d);
            },
            error: function (msg) {
        ...
        tree: function (options, value) {
            if (typeof (options) == 'string') {
                ...
                switch (options) {
                    ...
                    case 'load':
                        if ($.isArray(value)) {
                            ...
                            if (value.length == 0)
                                $('.ui-icon:first', selector).addClass('leaf');
                            else
							{
                                ...
                                $.each(value, function (i, node) {
                                    ...
                                    if(typeof(treeOptions.toggled)!='undefined')
                                        treeNode.bind('tree.toggled', treeOptions.toggled);
                                    if (typeof (treeOptions.children(node)) == 'undefined' || !treeOptions.children(node) || treeOptions.children(node).length == 0)
                                        $('> div:first', treeNode).addClass('leaf');
                ...
                if (typeof (options.loaded) != 'undefined')
                    this.bind('tree.loaded', options.loaded);
